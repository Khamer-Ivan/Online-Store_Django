...
class Order(models.Model):  # покупка товаров из корзины
    DELIVERY_CHOICES = [("reg", "Обычная доставка"), ("exp", "Экспресс доставка")]
    ...
    fio = models.CharField(
        max_length=100, null=True, blank=True, verbose_name="name and lastname"
    )
    phone = models.CharField(
        max_length=16, null=True, blank=True, verbose_name="phone number"
    )
    email = models.EmailField(null=True, blank=True, verbose_name="email")
    delivery = models.CharField(
        max_length=3, choices=DELIVERY_CHOICES, default="reg", verbose_name="delivery"
    )
    ...
    braintree_id = models.CharField(
        max_length=150, blank=True, verbose_name="transaction id"
    )
    delivery_cost = models.DecimalField(
        max_digits=10, decimal_places=2, default=0, verbose_name="delivery cost"
    )
    product_order = models.ForeignKey(
        "products.Product",
        on_delete=models.CASCADE,
